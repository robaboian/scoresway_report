...
Modo de uso:
- Carga los links del partido a analizar ingresando a [Scoresway](https://www.scoresway.com/en_GB/soccer) y [365Scores](https://www.365scores.com/es), y colocalos en sus respectivos campos. Si es Copa Argentina o no hay datos de 365Scores, podés generar el reporte solo con Scoresway.
...
def get_passes_df(df):
    # Algunos eventos (por ejemplo, carries insertados o filas incompletas) pueden
    # tener type = NaN. Sin na=False, str.contains devuelve NaN en esas filas y
    # el operador ~ no puede invertir esos valores float.
    excluded_types = r"SubstitutionOn|FormationChange|FormationSet|Card"
    type_text = df["type"].fillna("").astype(str)
    df_events = df.loc[~type_text.str.contains(excluded_types, regex=True, na=False)].copy()

    # El receptor se infiere a partir del jugador del siguiente evento válido.
    df_events["receiver"] = df_events["playerId"].shift(-1)

    pass_columns = [
        "index", "x", "y", "endX", "endY", "teamName", "playerId",
        "receiver", "type", "outcomeType", "pass_or_carry_angle"
    ]
    return df_events.loc[df_events["type"].eq("Pass"), pass_columns].copy()
...
def _validate_inputs(url_365scores: str, url_scoresway: str, use_365scores: bool = True) -> list[str]:
    errors = []
    if use_365scores:
    ...
    if not url_scoresway.strip():
        if use_365scores:
            ...
            errors.append("Las opciones de reporte aparecerán cuando el link de Scoresway esté cargado.")
    elif "scoresway.com" not in url_scoresway or "/match/" not in url_scoresway:
        errors.append("El link no parece ser un partido de Scoresway.")
    return errors
...
def _empty_365scores_bootstrap() -> str:
    return """# Modo sin 365Scores: se crean dataframes vacíos compatibles con los bloques de remates.
USAR_365SCORES = False
players365 = pd.DataFrame(columns=[
    "id", "name", "shortName", "jerseyNumber", "competitorId"
])
shots365 = pd.DataFrame(columns=[
    "line", "side", "y", "z", "playerId", "competitorNum", "shot_outcome",
    "type", "xg", "xgot", "time", "goalDescription"
])"""


def _patch_first_cell(source: str, url_365scores: str, url_scoresway: str, home_color: str, away_color: str, use_365scores: bool = True) -> str:
    if use_365scores:
        players_replacement = f"USAR_365SCORES = True\nplayers365 = threesixfivescores.get_players_info({url_365scores!r})"
        shots_replacement = f"shots365 = threesixfivescores.get_match_shotmap({url_365scores!r})"
    else:
        players_replacement = _empty_365scores_bootstrap()
        shots_replacement = "# shots365 ya fue creado vacío en modo sin 365Scores"

    replacements = [
        (r'players365\s*=\s*threesixfivescores\.get_players_info\("[^"]+"\)', players_replacement),
        (r'shots365\s*=\s*threesixfivescores\.get_match_shotmap\("[^"]+"\)', shots_replacement),
...
"""
    ...
    for step_number, (cell_index, source) in enumerate(code_cells, start=1):
        if cell_index == 0:
            source = _patch_first_cell(source, url_365scores, url_scoresway, home_color, away_color, use_365scores=use_365scores)
...
def _run_report_job(url_365scores: str, url_scoresway: str, selenium_wait: int, headless: bool, home_color: str, away_color: str, subtitle: str, log_placeholder, progress_bar, progress_text, mode: str = "group", selected_players: list[str] | None = None, use_365scores: bool = True) -> Path:
    workdir = _prepare_workdir()
    runner_path = workdir / "_scoresway_report_runner.py"
    runner_path.write_text(_build_runner_code(url_365scores, url_scoresway, selenium_wait, headless, home_color, away_color, subtitle, mode=mode, selected_players=selected_players, use_365scores=use_365scores), encoding="utf-8")
...
def _run_report_job_with_retry(url_365scores: str, url_scoresway: str, selenium_wait: int, headless: bool, home_color: str, away_color: str, subtitle: str, log_placeholder, progress_bar, progress_text, mode: str = "group", selected_players: list[str] | None = None, use_365scores: bool = True) -> Path:
    try:
        return _run_report_job(url_365scores, url_scoresway, selenium_wait, headless, home_color, away_color, subtitle, log_placeholder, progress_bar, progress_text, mode=mode, selected_players=selected_players, use_365scores=use_365scores)
        ...
        return _run_report_job(url_365scores, url_scoresway, max(selenium_wait + 12, 35), False, home_color, away_color, subtitle, log_placeholder, progress_bar, progress_text, mode=mode, selected_players=selected_players, use_365scores=use_365scores)
...
def _run_ui_job(mode: str, url_365scores: str, url_scoresway: str, headless: bool, home_color: str, away_color: str, subtitle: str, selected_players: list[str] | None = None, use_365scores: bool = True):
...
def main() -> None:
    ...
    use_365scores = not es_copa_argentina
    ...
    if use_365scores:
        ...
        url_365scores = ""
        st.info("Modo solo Scoresway activado: no se usará 365Scores. Los módulos dependientes de xG/xGOT/remates 365 quedarán vacíos o en cero.")
    ...
    if use_365scores and (not url_365scores.strip() or not url_scoresway.strip()):
        st.info("Las opciones de reporte aparecerán cuando ambos links estén cargados.")
        st.stop()
    if not use_365scores and not url_scoresway.strip():
        st.info("Las opciones de reporte aparecerán cuando el link de Scoresway esté cargado.")
        st.stop()

    errors = _validate_inputs(url_365scores, url_scoresway, use_365scores=use_365scores)
    if errors:
        for error in errors:
            st.warning(error)
        st.stop()
    ...
    if modalidad == "Individual":
        ...
        if st.button("Cargar datos y jugadores", type="secondary"):
            workdir = _run_ui_job("prepare", url_365scores, url_scoresway, headless, home_color, away_color, subtitle, use_365scores=use_365scores)
        ...
        if st.button("Generar reporte individual", type="primary", disabled=not selected_players):
            workdir = _run_ui_job("individual", url_365scores, url_scoresway, headless, home_color, away_color, subtitle, selected_players=selected_players, use_365scores=use_365scores)
            if workdir is not None:
        ...
        if st.button("Generar reporte grupal", type="primary"):
            workdir = _run_ui_job("group", url_365scores, url_scoresway, headless, home_color, away_color, subtitle, use_365scores=use_365scores)
            if workdir is not None:
